...
\hypersetup{
pdftitle={The Calculus of Rainbows},
pdfsubject={Calculus},
pdfauthor={Matt Alejo, Joshua de Luna, Althea Loyola},
pdfkeywords={calculus, rainbows, snell}
}
...
{
\begin{tikzpicture}[line cap=round,line join=round,>=triangle 45,x=1cm,y=1cm,scale=1.2]
\clip(-1.6481524516351462,-0.183021382422293) rectangle (5.512524834546647,4.7);
\draw [line width=2pt,domain=-1.6481524516351462:5.512524834546647] plot(\x,{(-0-0*\x)/1});
\draw[line width=0.75pt,draw=myviolet, smooth,samples=100,domain=0:5.512524834546647] plot(\x,{0.6145562845*(\x)});
\draw[line width=0.75pt,draw=myindigo, smooth,samples=100,domain=0:5.512524834546647] plot(\x,{0.620945365*(\x)});
\draw[line width=0.75pt,draw=myblue, smooth,samples=100,domain=0:5.512524834546647] plot(\x,{0.6257073969*(\x)});
...
is the path that can be traversed in the least time. In an ideal scenario -- in a vacuum -- the path of least time is also the path of least distance, the line, as described by the equation
...
As visulized in Figure \ref{fig:secondary}, this phenomenon results from the part of a ray that enters a raindrop and is refracted at $A$, reflected twice (at $B$ and $C$), and refracted as it leaves the drop at $D$. Whereas, in Section \ref{sec:primary}, $D(\alpha)$ refers to the amount of {\itshape clockwise} deviation, in this section about secondary rainbows, the deviation angle $D(\alpha)$ refers to the total amount of {\itshape counterclockwise} rotation that the ray undergoes in this four-stage process.\\
